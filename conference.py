...
FEATURED_TPL = "%s's Featured Sessions: %s"
...
SESSION_GET_BEFORE_TIME_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
# - - - Session Objects - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        # Require Authrization
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authrization required')
    ...
    def getConferenceSessions(self, request):
        """GET: Get all sessions in a conference"""
    ...
    def getConferenceSessionsByType(self, request):
        """GET: Get all sessions in a conference with given type"""
        sessions = Session.query(ancestor=ndb.Key(urlsafe=request.websafeConferenceKey))
    ...
    def getConferenceSessionsBySpeaker(self, request):
        """GET: Get all sessions in a conference with given speaker"""
        sessions = Session.query(ancestor=ndb.Key(urlsafe=request.websafeConferenceKey))
    ...
    def getSessionsByType(self, request):
        """GET: Get all sessions with given type"""
        sessions = []
        if getattr(request, 'typeOfSession'):
            sessions = Session.query().filter(Session.typeOfSession == request.typeOfSession)
        return SessionForms(items=[self._copySessionToForm(session) for session in sessions])
    ...
    def getSessionsBySpeaker(self, request):
        """GET: Get all sessions with given speaker"""
        sessions = []
...
# - - - Wishlist - - - - -
    ...
    def _copyWishlistToForm(self, session_wishlist):
        """Copy Wishlist Object into Form """
        sw = SessionWishListForm()
        for field in sw.all_fields():
            if hasattr(session_wishlist, field.name):
                setattr(sw, field.name, getattr(session_wishlist, field.name))
            elif field.name == "websafeKey":
                setattr(sw, field.name, session_wishlist.key.urlsafe())
        sw.check_initialized()
        return sw


    @endpoints.method(WISHLIST_POST_REQUEST, SessionWishListForm,
                      path='wishlist',
                      http_method='POST',
                      name='addSessionToWishlist')
    def addSessionToWishlist(self, request):
        """POST: Add Session into Wishlist"""
        ...
        w_key = ndb.Key(SessionWishList, request.websafeSessionKey)
        session_wishlist = w_key.get()
        if session_wishlist:
            raise endpoints.BadRequestException('Session already added')

        session_wishlist = SessionWishList(key = w_key,
                                           conferenceKey=s_key.parent().urlsafe(),
                                           sessionKey=s_key.urlsafe(),
                                           userID = user_id)
        session_wishlist.put()
        return self._copyWishlistToForm(session_wishlist)
    ...
    def getSessionsInWishlist(self, request):
        """
            GET: Get sessions in the Wishlist
            Input: conferenceKey(Optional)
                - Not Provided: List all the sessions in the wishlist
                - Provided:
                    List all the sessions in the conference in the wishlist
        """
        # Require Authorization
        user = endpoints.get_current_user()
        if not user:
        ...
        sw = SessionWishList.query(SessionWishList.userID == user_id)
        # If conference key is provided, futher filter down
        if hasattr(request, 'websafeConferenceKey'):
            if request.websafeConferenceKey != None:
                sw = sw.filter(SessionWishList.conferenceKey == \
                               request.websafeConferenceKey)

        sessions = []
        if sw:
            sessionkeys = [ndb.Key(urlsafe=w.sessionKey) for w in sw]
...
# - - - Feature Speaker - - -
    ...
    def getFeaturedSpeaker(self, request):
        """GET: Shows Memcache(Featured Speker)"""
